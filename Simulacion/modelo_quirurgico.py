...
RUTA_EXCEL = path.join("..","Capstone-Equipo-11","Simulacion","Datos","Datos Operaciones y lista de espera.xlsx")
...
# Recursos
N_PABELLONES = 8            # |J|
CAP_CAMAS_BASICAS = 30      # C^BAS — ajustar según supuesto del hospital
...
TIEMPO_LIMITE_SEG = 600     # 10 minutos
MIP_GAP = 0.01              # detener cuando el gap sea menor al 1%
...
# usando la mediana del histórico agrupando por descripción de la cirugía.
...
if m.SolCount == 0:
    ...
    for (i, j, d, s) in combinaciones_validas:
        if x[i, j, d, s].X > 0.5:
            ...
            asignaciones.append({
                "Paciente": df_espera.loc[i, "ID paciente"],
                "Servicio": g[i],
